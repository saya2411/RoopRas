...
export const generateRandomNotionFace = async (modelName: string): Promise<string | null> => {
  ...
  for (let i = 0; i < apiKeys.length; i++) {
    ...
    // For each key, try the primary model and then the fallback model
    for (const model of modelsToTry) {
      try {
        ...
        if (isRateLimit || isNoQuota) {
          console.warn(`Key ${index + 1} with model ${model} is unusable. Trying next option...`);
          continue; // Try next model for this key, or next key
        }
        
        console.error(`Key ${index + 1} failed with a non-quota error:`, error);
        throw error;
      }
    }
  }
