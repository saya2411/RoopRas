...
export const generateRandomNotionFace = async (): Promise<any> => {
  ...
  for (const modelName of modelsToTry) {
    try {
      const headShapes = [
        "a rounded square head", 
        "an irregular blob-like head", 
        "a slightly squarish head with soft corners", 
        "a perfect circle head",
        "a perfectly round circular head",
      ...
      const prompt = `A unique, random Notion Faces style avatar. The head shape MUST be ${randomHeadShape}. Features include ${randomEyeStyle} and ${randomMouthStyle}. The style is minimalist, abstract, with bold, clean black outlines. Shading achieved solely through stippling. Strictly pure black and pure white. NO colors. NO background circle, NO circular frame. Just the face itself on a plain white background.`;
